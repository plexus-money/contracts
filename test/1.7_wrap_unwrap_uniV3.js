...
describe('Deploying the plexus contracts for WrapperUni adding liquidity test', () => {
  ...
  describe('Test Uni V3 liquidity pool', () => {
    ...
    it('Should convert 2 ETH to USDT Token(s) from MakerDao via Uniswap', async () => {
      ...
      if (status === 1) {
        ...
        const daiTokenBalance = Number(ethers.utils.formatUnits(await daiToken.balanceOf(addr1.address), 6));
    ...
    it('Should wrap USDT Token(s) to (ETH-USDT) via UniswapV3', async () => {
      ...
      const encodedKey = ethers.utils.defaultAbiCoder.encode(types, values)
      //const encodedKey = ethers.utils.defaultAbiCoder.encodePacked(usdtTokenAddress,3000, zeroAddress)
      //abi.encodePacked(DAI, poolFee, WETH9);
      const incentiveId = ethers.utils.keccak256(encodedKey)
      ...
      let daiTokenBalance = await daiToken.balanceOf(addr1.address);
      // Convert the 2 ETH to Dai Token(s)
      const deadline = Math.floor(new Date().getTime() / 1000) + 1000;
      const paths = [usdtTokenAddress, zeroAddress];
      const path = [incentiveId, "0xdac17f958d2ee523a2206206994597c13d831ec7000bb8c02aaa39b223fe8d0a0e5c4f27ead9083c756cc2"];
      console.log(path[0]);
      let wrapParams = {
        sourceToken: usdtTokenAddress,
        destinationTokens: paths,
        paths: path,
        amount: daiTokenBalance,
        minAmounts: [0, 0],
        poolFee: 3000,
        tickLower: tickLower,
        tickUpper: tickUpper,
      ...
      await daiToken.connect(addr1).approve(wrapper.address,daiTokenBalance );
      const { status } = await (await wrapper.connect(addr1).wrap(wrapParams)).wait();
      expect(status).to.equal(1);
    ...
    it('Should return USDT and ETH after unwrapping the liquidity nft via univ3', async () => {
      const userSlippageTolerance = config.userSlippageTolerance;
      const zeroAddress = process.env.ZERO_ADDRESS;
      ...
      await positionManager_contract.connect(addr1).approve(wrapper.address, 134216);
