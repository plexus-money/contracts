...
describe('Deploying the plexus contracts for WrapperUni remix test', () => {
  ...
  describe('Testing Uni V2 remixing liquidity', () => {
      ...
      it('Should create pool(SUSHI-COMPOUND) with DAI via UniswapV2', async () => {
          ...
          const amountOutMin1 = await getAmountOutMin(path1, BigNumber.from(amountPlaceholder).div(2), userSlippageTolerance, wrapper, 18);
          const amountOutMin2 = await getAmountOutMin(path2, BigNumber.from(amountPlaceholder).div(2), userSlippageTolerance, wrapper, 18);
      ...
      it('Should do a cross-dex remix for the (SUSHI-COMPOUND) LP Token to the (ETH-USDC) LP Token in Sushi', async () => {
        const userSlippageTolerance = config.userSlippageTolerance;
        ...
        log("Initial LP Token balance for (SUSHI-COMPOUND) from Uni v2 is greater than zero: ", initialLpTokenBalance);
        ...
        const unwrapAmountOutMin1 = await getAmountOutMin(unwrapPath1, BigNumber.from(amountPlaceholder).div(2), userSlippageTolerance, wrapper, 18);
        const unwrapAmountOutMin2 = await getAmountOutMin(unwrapPath2, BigNumber.from(amountPlaceholder).div(2), userSlippageTolerance, wrapper, 18);
        ...
        const wrapAmountOutMin1 = await getAmountOutMin(wrapPath1, BigNumber.from(amountPlaceholder).div(2), userSlippageTolerance, wrapper, 18);
        const wrapAmountOutMin2 = await getAmountOutMin(wrapPath2, BigNumber.from(amountPlaceholder).div(2), userSlippageTolerance, wrapper, 18);
        ...
        const { status, events } = await (await wrapper
          ...
          if (status === 1) {
              ...
              log("Remixed LP Token pair address for (ETH-USDC) in Sushi: ", remixedTokenPairAddress);

              const lpToken = new ethers.Contract(remixedTokenPairAddress, abi, provider);
              const lpTokenBalance = Number(ethers.utils.formatUnits(await lpToken.balanceOf(owner.address), `ether`));
              log("Remixed LP Token balance for (ETH-USDC) in Sushi: ", lpTokenBalance);
              expect(lpTokenBalance).to.be.gt(0);
              ...
              log("Final LP Token balance for (SUSHI-COMPOUND) from Uni v2 should be zero after remix: ", initialLpTokenBalance);
