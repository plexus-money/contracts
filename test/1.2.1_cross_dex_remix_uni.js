...
describe('Deploying the plexus contracts for WrapperUni remix test', () => {
  ...
  describe('Testing Uni V2 remixing liquidity', () => {
      ...
      it('Should do a cross-dex remix for the (SUSHI-COMPOUND) LP Token to the (ETH-USDC) LP Token in Sushi', async () => {
        ...
        const unwrapAmountOutMin1 = await getAmountOutMin(unwrapPath1, amountPlaceholder, userSlippageTolerance, wrapper);
        const unwrapAmountOutMin2 = await getAmountOutMin(unwrapPath2, amountPlaceholder, userSlippageTolerance, wrapper);
        ...
        const wrapAmountOutMin1 = await getAmountOutMin(wrapPath1, amountPlaceholder, userSlippageTolerance, wrapper);
        const wrapAmountOutMin2 = await getAmountOutMin(wrapPath2, amountPlaceholder, userSlippageTolerance, wrapper);
