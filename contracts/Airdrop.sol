...
contract Airdrop is OwnableUpgradeable {
    ...
    function claimAirdropWithOption1() external {
        ...
        uint256 upfrontAmount = (4 * amounts[msg.sender]) / 10;
    ...
    function claimAirdropWithOption2() external {
        ...
        if (sentAmounts[msg.sender] > 0) {
            uint256 epochs = (block.number - blockNumbers[msg.sender]) /
                blocksForEpoch;
            ...
            uint256 upfrontAmount = (3 * amounts[msg.sender]) / 10;
            uint256 amount = ((epochs * amounts[msg.sender]) / 10) -
                (sentAmounts[msg.sender] - upfrontAmount);
            token.safeTransfer(msg.sender, amount);
            sentAmounts[msg.sender] += amount;
        } else {
            sentAmounts[msg.sender] = (3 * amounts[msg.sender]) / 10;
